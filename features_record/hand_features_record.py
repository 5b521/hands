...
def features_get(detector : htm, hand_num = 0):
    ...
    FDirection = detector.direction(detector.results.multi_handedness[hand_num].classification[0].label)
        ...
        if len(land_mark_List) != 0 and key.is_pressed(' '):
            ...
            for i in range(0,len(detector.results.multi_hand_landmarks)):
                ...
                if record_features.get(left_right):
                    ...
                    record_features[detector.results.multi_handedness[i].classification[0].label] = curr_features
            
            
            cv2.putText(img, 'good Q,continue E', (10, 70), cv2.FONT_HERSHEY_PLAIN, 3, (255, 0, 255), 3)
